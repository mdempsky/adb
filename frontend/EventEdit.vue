  ...
  $.ajax({
    ...
    success: data => {
      ...
      for (let name of data.activist_names) {
        ACTIVIST_NAMES.push(name);
        ACTIVIST_NAMES_SET.add(name);
      }
    },
    error: () => {
...
export default {
  ...
  methods: {
    ...
    changed(x, y) {
      ...
      // If last row is non-empty, add another.
      const last = this.attendees[this.attendees.length - 1].trim();
      if (last != "") {
        this.addRows(1);
      ...
      for (let i = 0; i < this.attendees.length; i++) {
        ...
        let warning = "";
        if (name != "") {
          if (!ACTIVIST_NAMES_SET.has(name)) {
            warning = "unknown";
